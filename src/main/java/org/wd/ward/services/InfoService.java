...
import org.wd.ward.dto.*;
import org.wd.ward.exceptions.ApplicationNotSetUpException;
...
public class InfoService
{
    ...
    private ProcessorDto getProcessor()
    {
        ...
        processorDto.setCoreCount(coreCount + ((coreCount > 1) ? " 個核心" : " 個核心"));
        ...
        processorDto.setBitDepth(BitDepthPrefix + "位元");
    ...
    private MachineDto getMachine()
    {
        ...
        machineDto.setTotalRam(getConvertedCapacity(globalMemory.getTotal()) + " 内存");
        ...
        {
            machineDto.setRamTypeOrOSBitDepth(operatingSystem.getBitness() + "位元");
        }

        int processCount = operatingSystem.getProcessCount();
        machineDto.setProcCount(processCount + ((processCount > 1) ? " 個程序" : " 個程序"));
    ...
    private StorageDto getStorage()
    {
        ...
        {
            storageDto.setMainStorage("未定義");
        }

        long total = hwDiskStores.stream().mapToLong(HWDiskStore::getSize).sum();
        storageDto.setTotal("總計" + getConvertedCapacity(total));

        int diskCount = hwDiskStores.size();
        storageDto.setDiskCount(diskCount + ((diskCount > 1) ? " 顆硬碟" : " 顆硬碟"));

        storageDto.setSwapAmount(getConvertedCapacity(globalMemory.getVirtualMemory().getSwapTotal()) + " 交換區");
